...
当前大部份客户端&服务端架构的应用程序，都是用http/1.1连接的，现代浏览器与单个域最大连接数，都在4-6个左右，由上图Total Requests数据，如果不用CDN分流，平均有20个串行请求。
...
### 2. 多路复用
HTTP/2 复用TCP连接，在一个连接里，客户端和浏览器都可以同时发送多个请求或回应，而且不用按照顺序一一对应，这样就避免了"队头堵塞"（见TCP/IP详解）。
...
那么存在一个问题，如果客户端设置了缓存怎么办。按照RFC文档有三种方式
...
Fiddler是直接作为中间代理，可以作为客户端直接与服务端通讯，可以像浏览器那样直接解密https，可以直接看到https报文,
...
## 最后
最后，HTTP2提供了更高的传输速度，更少的资源占用，可以去除各种性能优化tricks(如css sprite,inline-image.)
转向WEB开发的美好未来T.T
